...
export default function GoodsList() {
  ...
  // 过滤商品（使用 getRealStatus 与 GoodsCard 显示保持一致）
  const getFilteredGoods = () => {
    let filtered = [...goodsList];

    // 状态过滤（按实时计算的状态而非数据库静态字段）
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (g) => getRealStatus(g) === Number(statusFilter),
      );
    }
    ...
    switch (sortBy) {
      ...
      case "discount":
        ...
        break;
      default:
        // 默认按实时状态排序：进行中 > 未开始 > 已结束
        filtered.sort((a, b) => {
          const order = { 1: 0, 0: 1, 2: 2, 3: 3 };
          const statusA = getRealStatus(a);
          const statusB = getRealStatus(b);
          return (
            (order[statusA as keyof typeof order] || 9) -
            (order[statusB as keyof typeof order] || 9)
          );
        });
    }
  ...
  // Tab 选项（使用 getRealStatus 与卡片显示一致）
  const tabItems = [
    { key: "all", label: `全部 (${goodsList.length})` },
    {
      key: String(GoodsStatus.IN_PROGRESS),
      label: `${GoodsStatusText[GoodsStatus.IN_PROGRESS]} (${
        goodsList.filter((g) => getRealStatus(g) === GoodsStatus.IN_PROGRESS)
          .length
      })`,
    },
    {
      key: String(GoodsStatus.NOT_STARTED),
      label: `${GoodsStatusText[GoodsStatus.NOT_STARTED]} (${
        goodsList.filter((g) => getRealStatus(g) === GoodsStatus.NOT_STARTED)
          .length
      })`,
    },
    {
      key: String(GoodsStatus.ENDED),
      label: `${GoodsStatusText[GoodsStatus.ENDED]} (${
        goodsList.filter((g) => getRealStatus(g) === GoodsStatus.ENDED).length
      })`,
    },
  ];
