# 🚀 Cloud Seckill - 高并发秒杀系统

![Java](https://img.shields.io/badge/Java-17-orange?style=flat-square&logo=openjdk)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.2.4-brightgreen?style=flat-square&logo=springboot)
![Spring Cloud](https://img.shields.io/badge/Spring%20Cloud-2023.0.1-blue?style=flat-square&logo=spring)
![License](https://img.shields.io/badge/License-MIT-yellow?style=flat-square)

## 📖 项目简介

**Cloud Seckill** 是一个基于 **Spring Cloud Alibaba** 的分布式高并发秒杀系统，旨在解决电商秒杀场景下的高并发、高可用、数据一致性等核心问题。项目采用微服务架构设计，通过多种技术手段实现秒杀业务的高性能处理。

## 🏗️ 系统架构

```
                            ┌─────────────────┐
                            │   用户请求入口   │
                            └────────┬────────┘
                                     │
                            ┌────────▼────────┐
                            │  API Gateway    │
                            │  (seckill_gateway)
                            └────────┬────────┘
                                     │
              ┌──────────────────────┼──────────────────────┐
              │                      │                      │
     ┌────────▼────────┐    ┌────────▼────────┐    ┌────────▼────────┐
     │   订单服务       │    │   库存服务       │    │   其他服务...    │
     │ (seckill_order) │    │ (seckill_stock) │    │                 │
     └────────┬────────┘    └────────┬────────┘    └─────────────────┘
              │                      │
              └──────────┬───────────┘
                         │
        ┌────────────────┼────────────────┐
        │                │                │
   ┌────▼────┐     ┌─────▼─────┐    ┌─────▼─────┐
   │  MySQL  │     │   Redis   │    │ RocketMQ  │
   └─────────┘     └───────────┘    └───────────┘
```

## 📦 模块说明

| 模块名称 | 描述 |
|---------|------|
| `seckill_common` | 公共模块，包含通用工具类、常量、响应封装等 |
| `seckill_gateway` | API 网关模块，负责请求路由、负载均衡、限流等 |
| `seckill_order` | 订单服务模块，处理秒杀订单的创建与管理 |
| `seckill_stock` | 库存服务模块，管理商品库存的扣减与同步 |

## 🛠️ 技术栈

### 核心框架
- **Java 17** - JDK 版本
- **Spring Boot 3.2.4** - 基础框架
- **Spring Cloud 2023.0.1** - 微服务框架
- **Spring Cloud Alibaba 2023.0.1.0** - 阿里巴巴微服务组件

### 中间件
| 组件 | 版本 | 用途 |
|------|------|------|
| **Nacos** | v2.3.1 | 服务注册与发现、配置中心 |
| **MySQL** | 8.0 | 持久化存储 |
| **Redis** | 7.2 | 缓存、分布式锁、库存预扣减 |
| **RocketMQ** | 5.2.0 | 消息队列、异步削峰 |
| **Sentinel** | 1.8.6 | 流量控制、熔断降级 |

### 持久层 & 工具
- **MyBatis-Plus 3.5.7** - ORM 框架
- **Hutool 5.8.26** - Java 工具库
- **Knife4j 4.5.0** - API 文档生成

## 🚀 快速开始

### 环境要求

- JDK 17+
- Maven 3.8+
- Docker & Docker Compose

### 1. 启动基础设施

使用 Docker Compose 一键启动所有中间件：

```bash
# 进入项目根目录
cd cloud_seckill

# 启动所有服务
docker-compose up -d
```

### 2. 服务端口说明

| 服务 | 端口 | 访问地址 |
|------|------|----------|
| Nacos | 8848 | http://localhost:8848/nacos |
| MySQL | 3307 | localhost:3307 (root/root) |
| Redis | 6379 | localhost:6379 (密码: root) |
| RocketMQ NameServer | 9876 | localhost:9876 |
| RocketMQ Dashboard | 8081 | http://localhost:8081 |
| Sentinel Dashboard | 8858 | http://localhost:8858 (sentinel/sentinel) |

### 3. 编译项目

```bash
# Maven 编译
mvn clean install -DskipTests
```

### 4. 启动微服务

按以下顺序启动各个服务：

1. **seckill_gateway** - 网关服务
2. **seckill_stock** - 库存服务
3. **seckill_order** - 订单服务

## 📁 项目结构

```
cloud_seckill/
├── data/                    # Docker 数据持久化目录
│   └── mysql/              # MySQL 数据文件
├── seckill_common/          # 公共模块
│   └── pom.xml
├── seckill_gateway/         # 网关模块
│   └── pom.xml
├── seckill_order/           # 订单服务模块
│   └── pom.xml
├── seckill_stock/           # 库存服务模块
│   └── pom.xml
├── docker-compose.yml       # Docker 编排文件
├── pom.xml                  # 父工程 POM
└── README.md               # 项目文档
```

## 🔧 配置说明

### MySQL 配置

- **主机**: localhost
- **端口**: 3307
- **用户名**: root
- **密码**: root
- **默认数据库**: seckill_order (可自行创建 seckill_stock 库)

### Redis 配置

- **主机**: localhost
- **端口**: 6379
- **密码**: root
- **持久化**: 开启 AOF

### RocketMQ 配置

- **NameServer**: localhost:9876
- **Broker**: localhost:10911

## ✨ 核心特性

- 🔐 **分布式锁** - 基于 Redis 实现库存扣减的原子性操作
- 📨 **异步削峰** - 通过 RocketMQ 消息队列异步处理订单
- 🛡️ **流量控制** - 集成 Sentinel 实现限流、熔断、降级
- 🔄 **服务发现** - 基于 Nacos 的服务注册与发现
- ⚙️ **配置中心** - 基于 Nacos 的分布式配置管理
- 🚪 **API 网关** - Spring Cloud Gateway 统一入口

## 📊 中间件控制台

| 控制台 | 地址 | 账号/密码 |
|--------|------|-----------|
| Nacos 控制台 | http://localhost:8848/nacos | nacos/nacos |
| RocketMQ Dashboard | http://localhost:8081 | - |
| Sentinel Dashboard | http://localhost:8858 | sentinel/sentinel |

## 🤝 贡献指南

1. Fork 本项目
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 提交 Pull Request

## 📄 开源协议

本项目采用 [MIT License](LICENSE) 开源协议。

## 📞 联系方式

如有问题或建议，欢迎提交 Issue 或 Pull Request。

---

<p align="center">
  <b>⭐ 如果这个项目对你有帮助，请给一个 Star 支持一下！</b>
</p>
